...
LOSS_FN = 'Focal' # Can be 'Focal' or 'CrossEntropy'
...
if DATASET == 'Vaihingen':
    ...
    DSM_FOLDER = MAIN_FOLDER + 'ISPRS_semantic_labeling_Vaihingen/dsm/dsm_09cm_matching_area{}.tif'
    LABEL_FOLDER = MAIN_FOLDER + 'ISPRS_semantic_labeling_Vaihingen/gts_for_participants/top_mosaic_09cm_area{}.tif'
    BOUNDARY_FOLDER = MAIN_FOLDER + 'top_Boundary/top_mosaic_09cm_area{}_Boundary.tif'
    OBJECT_FOLDER = MAIN_FOLDER + 'top_objects/top_mosaic_09cm_area{}_objects.tif'
    ERODED_FOLDER = MAIN_FOLDER + 'ISPRS_semantic_labeling_Vaihingen_ground_truth_eroded_COMPLETE/top_mosaic_09cm_area{}_noBoundary.tif'
elif DATASET == 'Urban':
...
class ISPRS_dataset(torch.utils.data.Dataset):
    def __init__(self, ids, data_files=DATA_FOLDER, label_files=LABEL_FOLDER,
        ...
        files_to_check = self.data_files + self.label_files
        if self.dsm_files:
            files_to_check += self.dsm_files
    ...
    @classmethod
    def data_augmentation(cls, rgb, dsm, boundary, objects, label, flip=True, mirror=True, rotate=True, color_jitter=True):
        
        # Color Jitter (only on RGB)
        if color_jitter and random.random() < 0.5:
            # Convert numpy array (C, H, W) to PIL Image
            ...
            # Convert back to numpy array (C, H, W)
            rgb = np.asarray(rgb_pil).astype('float32') / 255
        ...
        if rotate and random.random() < 0.5:
            k = random.randint(1, 3) # 1, 2, or 3 corresponds to 90, 180, 270 deg
            rgb = np.rot90(rgb, k, axes=(1, 2))
            dsm = np.rot90(dsm, k, axes=(1, 2))
            boundary = np.rot90(boundary, k, axes=(0, 1))
            objects = np.rot90(objects, k, axes=(0, 1))
            label = np.rot90(label, k, axes=(0, 1))

        return np.copy(rgb), np.copy(dsm), np.copy(boundary), np.copy(objects), np.copy(label)
    ...
    def __getitem__(self, i):
        ...
        if self.dsm_files:
            if random_idx in self.dsm_cache_.keys():
                ...
                dsm = io.imread(self.dsm_files[random_idx])
                if len(dsm.shape) == 2:  # Check if single channel
                    dsm = np.expand_dims(dsm, axis=0)  # Add channel dim: (1, H, W)
                else:
                    dsm = dsm.transpose((2, 0, 1))[0:1, :, :]
        ...
        if dsm is not None:
            ...
            dsm_p = np.zeros_like(data_p[0:1,:,:])
        boundary_p = boundary[x1:x2, y1:y2]
        object_p = object[x1:x2, y1:y2]
        label_p = label[x1:x2, y1:y2]

        # Data augmentation
        data_p, dsm_p, boundary_p, object_p, label_p = self.data_augmentation(data_p, dsm_p, boundary_p, object_p, label_p)
...
class FocalLoss(nn.Module):
    ...
    def forward(self, predict, target, weight=None):
        ...
        # Calculate cross-entropy loss without reduction to get per-pixel loss.
        ce_loss = F.cross_entropy(predict, target, weight=weight, ignore_index=self.ignore_index, reduction='none')
        ...
        # If there are no valid (non-ignored) pixels, return a zero loss
        if not mask.any():
            return torch.tensor(0., device=predict.device, requires_grad=True)

        # Apply mask and then average over the valid pixels only
        focal_loss_masked = focal_loss[mask]
        
        return focal_loss_masked.mean()

def loss_calc(pred, label, weights):
    """
    This function returns the chosen loss for semantic segmentation.
    It can be configured to use either Focal Loss or standard Cross Entropy.
    """
    label = Variable(label.long()).cuda()
    
    if LOSS_FN == 'Focal':
        # gamma=2 is a commonly used and effective value
...
class BoundaryLoss(nn.Module):
    ...
    def forward(self, pred, gt):
        ...
        class_map = pred.argmax(dim=1)  # Get Class Map with the Shape: [B, H, W]

        # boundary map
        gt_b = F.max_pool2d(
            1 - (gt > 0).float().unsqueeze(1), kernel_size=self.theta0, stride=1, padding=(self.theta0 - 1) // 2)
        gt_b -= 1 - (gt > 0).float().unsqueeze(1)

        pred_b = F.max_pool2d(
            1 - (class_map > 0).float().unsqueeze(1), kernel_size=self.theta0, stride=1, padding=(self.theta0 - 1) // 2)
        pred_b -= 1 - (class_map > 0).float().unsqueeze(1)
